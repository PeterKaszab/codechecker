...
"""
This module tests the correctness of the MSBuildAnalyzerResult, which
used in sequence transform MSBuild output to a plist file.
"""
...
from codechecker_report_converter.analyzers.msbuild import analyzer_result
from codechecker_report_converter.report.parser import plist


class MSBuildAnalyzerResultTestCase(unittest.TestCase):
    """ Test the output of the MSBuildAnalyzerResult. """
    ...
    def setUp(self):
        ...
        self.test_files = os.path.join(os.path.dirname(__file__),
                                       'msbuild_output_test_files')

    def tearDown(self):
        """ Clean temporary directory. """
        shutil.rmtree(self.cc_result_dir)

    def test_no_msbuild_output_file(self):
    ...
    def test_transform_single_file(self):
        ...
        analyzer_result = os.path.join(self.test_files, 'reports.log')
        self.analyzer_result.transform(
            analyzer_result, self.cc_result_dir, plist.EXTENSION,
            file_name="{source_file}_{analyzer}")

        plist_file = os.path.join(self.cc_result_dir,
                                  'Sample.cs_msbuild.plist')
